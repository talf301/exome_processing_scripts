...
parser.add_option("--indels_out", dest="indels_out", help="Write indels and other not found variants to file (default 'spidex_indels_out.vcf')",default="spidex_indels_out.vcf")
...
short_header = ["Chrom","Pos","Ref","Alt","dpsi_max_tissue","dpsi_zscore", "gene","strand","transcript","exon_number","location","cds_type","ss_dist commonSNP_rs#"]
...
for line in sys.stdin:
  ...
  if not fields[fchr].startswith('chr'):
	  chrom = 'chr' + fields[fchr]
  else:
    chrom = fields[fchr]
  pos = int(fields[fpos])
